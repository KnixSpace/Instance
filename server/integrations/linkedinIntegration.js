...
router.get("/register",(req,res)=>{
    const authUrl = `${LINKEDIN_OAUTH_URL}?response_type=code&client_id=${process.env.LINKEDIN_CLIENT_ID}&redirect_uri=${process.env.LINKEDIN_CALLBACK_URI}&state=123456&scope=${LINKEDIN_SCOPES.join(" ")}`;
...
router.get("/callback",async(req,res)=>{
    ...
    try{
            ...
            await handleIntegration(req.user.userId,profileData.sub,profileData.email,profileData.picture,tokens);
...
async function updateExistingIntegration(intigration,accountId,email,avatar,tokens) {
